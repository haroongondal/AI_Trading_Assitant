...
"""Provision the AWS backend host for the AI Trading Assistant."""
...
def ensure_role(iam, role_name: str) -> None:
    ...
    try:
        ...
        iam.create_role(
            ...
            Description="EC2 role for AI Trading Assistant backend",
        )
...
def ensure_security_group(ec2, vpc_id: str, group_name: str, tags: dict[str, str]) -> str:
    ...
    if existing:
        ...
        response = ec2.create_security_group(
            ...
            Description="AI Trading Assistant backend security group",
            VpcId=vpc_id,
            TagSpecifications=[
                {
        ...
        BlockDeviceMappings=[
            {
                "DeviceName": "/dev/sda1",
                "Ebs": {
                    "VolumeSize": config.get("root_volume_gb", 20),
                    "VolumeType": "gp3",
...
def main() -> None:
    ...
    parser.add_argument("--config", required=True, help="Path to the AWS JSON config")
    ...
    image_id = resolve_ubuntu_ami(ssm)
    ...
    for _ in range(3):
        try:
            ...
            time.sleep(10)
    if instance is None:
        raise RuntimeError("Failed to launch AWS backend instance after retries.") from last_error
